...
switch($_GET[act]){
  ...
  echo"<div class='rightpanel'>
    ...
    $tampil = mysqli_query($query, "SELECT * FROM berita,kategori 
                           WHERE berita.id_kategori=kategori.id_kategori ORDER BY berita.id_berita DESC");}
	  
	  
    else{
    $tampil=mysqli_query($query, "SELECT * FROM berita WHERE username='$_SESSION[namauser]' ORDER BY id_berita DESC");}
						   $no = 1;
				...
				<form class='stdform stdform2' method=POST action='$aksi?module=berita&act=input' enctype='multipart/form-data'>
							...
							<p>
								...
								echo"<option value=0 selected>Pilih Kategori</option>";
   								$tampil=mysqli_query($query, "SELECT * FROM kategori ORDER BY nama_kategori");
   								while($r=mysqli_fetch_array($tampil)){
								...
								{
								$tampil=mysqli_query($query, "SELECT * FROM kategori WHERE username='$_SESSION[namauser]' ORDER BY nama_kategori");
   								while($r=mysqli_fetch_array($tampil)){
		   						echo "<option value=$r[id_kategori]>$r[nama_kategori]</option></p>"; 
   								}
								echo "</select></p>";
   ...
   echo "<p>
							...
							<p>
                                <label>Tag</label>
                                <span class='field'>";
   $tag = mysqli_query($query, "SELECT * FROM tag ORDER BY tag_seo"); 
   ...
   echo "
    ...
    break;  
	case "editberita":
 $edit = mysqli_query($query, "SELECT * FROM berita WHERE id_berita='$_GET[id]'");
               ...
               <div class='widgetbox box-inverse'>
                ...
                <span class='field'><input type='text' name='youtube' value='$r[youtube]' id='firstname2' class='input-xxlarge' /><small></br>contoh link: <span class style=\"color:#EA1C1C;\">http://www.youtube.com/embed/xbuEmoRWQHU</small></span>
							...
							<p>
								...
								<span class='field'><select name='kategori' id='selection2' class='uniformselect'>";
   $tampil=mysqli_query($query, "SELECT * FROM kategori ORDER BY id_kategori");
   if ($r[id_kategori]==0){
   echo "<option value=0 selected>- Pilih Kategori -</option>"; }   
