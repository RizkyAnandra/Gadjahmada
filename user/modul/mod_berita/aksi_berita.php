...
if ($module=='berita' AND $act=='hapus'){
  $data=mysqli_fetch_array(mysqli_query($query,"SELECT gambar FROM berita WHERE id_berita='$_GET[id]'"));
  if ($data[gambar]!=''){
     mysqli_query($query, "DELETE FROM berita WHERE id_berita='$_GET[id]'");
...
elseif ($module=='berita' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
   ...
   mysqli_query($query, "INSERT INTO berita( judul,
                                    sub_judul,
									youtube,
                                    judul_seo,
                                    id_kategori,
  ...
  $jml=count($tag_seo);
  for($i=0;$i<$jml;$i++){
    mysqli_query($query, "UPDATE tag SET count=count+1 WHERE tag_seo='$tag_seo[$i]'");
  }
}
// Update berita
elseif ($module=='berita' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
    mysqli_query($query, "UPDATE berita  berita SET judul       = '$_POST[judul]',
  ...
  header('location:../../media.php?module='.$module);
    ...
    $data_gambar = mysqli_query($query, "SELECT gambar FROM berita WHERE id_berita='$_POST[id]'");
	...
	@unlink('../../../foto_berita/'.'small_'.$r['gambar']);
     ...
     mysqli_query($query, "UPDATE berita SET judul       = '$_POST[judul]',
	                           sub_judul       = '$_POST[sub_judul]',
							      youtube      = '$_POST[youtube]',
                                   judul_seo   = '$judul_seo', 
                                   id_kategori = '$_POST[kategori]',
