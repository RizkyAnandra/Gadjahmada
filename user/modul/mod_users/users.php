...
switch($_GET[act]){
  ...
  echo"<div class='rightpanel'>
      ...
      $tampil = mysqli_query($query, "SELECT * FROM users ORDER BY id_session DESC");
    }
    else{
      $tampil=mysqli_query($query, "SELECT * FROM users WHERE username='$_SESSION[namauser]'");
    }
						   $no = 1;
	...
	case "editusers":
   $edit = mysqli_query($query, "SELECT * FROM users WHERE username='$_GET[username]'");
