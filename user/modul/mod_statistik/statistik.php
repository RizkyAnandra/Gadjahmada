  ...
  echo"<div class='rightpanel'>
        ...
        <div class='maincontent'>
            <div class='maincontentinner'>
              ...
              $s = mysqli_query($query, "SELECT * FROM statistik WHERE ip='$ip' AND tanggal='$tanggal'");
              // Kalau belum ada, simpan data user tersebut ke database
              if(mysqli_num_rows($s) == 0){
                mysqli_query($query, "INSERT INTO statistik(ip, tanggal, hits, online) VALUES('$ip','$tanggal','1','$waktu')");
              } 
              else{
                mysqli_query($query, "UPDATE statistik SET hits=hits+1, online='$waktu' WHERE ip='$ip' AND tanggal='$tanggal'");
              }

              $pengunjung       = mysqli_num_rows(mysqli_query($query, "SELECT * FROM statistik WHERE tanggal='$tanggal' GROUP BY ip"));
              $totalpengunjung  = mysqli_result(mysqli_query($query, "SELECT COUNT(hits) FROM statistik"), 0); 
              $hits             = mysqli_fetch_assoc(mysqli_query("SELECT SUM(hits) as hitstoday FROM statistik WHERE tanggal='$tanggal' GROUP BY tanggal")); 
              $totalhits        = mysqli_result(mysqli_query($query, "SELECT SUM(hits) FROM statistik"), 0); 
              $tothitsgbr       = mysqli_result(mysqli_query($query, "SELECT SUM(hits) FROM statistik"), 0); 
              $bataswaktu       = time() - 300;
              $pengunjungonline = mysqli_num_rows(mysqli_query($query, "SELECT * FROM statistik WHERE online > '$bataswaktu'"));
