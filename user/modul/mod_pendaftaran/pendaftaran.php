...
switch($_GET[act]){
  ...
  default:
   echo"<div class='rightpanel'>
    ...
    $tampil=mysqli_query($query, "SELECT * FROM pendaftaran ORDER BY id_pendaftaran");
    ...
    echo "</tbody></table>";
    $jmldata=mysqli_num_rows(mysqli_query($query, "SELECT * FROM pendaftaran"));
  ...
  case "editpendaftaran":
   $edit = mysqli_query($query, "SELECT *  FROM pendaftaran WHERE id_pendaftaran='$_GET[id]'");
       ...
       <div class='pageheader'>
        ...
        <div class='maincontent'>
		 ...
		 <select name='asal_sekolah' class='uniformselect'>";
		  $tampil=mysqli_query($query, "SELECT * FROM sekolah WHERE aktif='Y' ORDER BY nama_sekolah DESC");
		   if ($r[asal_sekolah]==0){
