...
$tampil = mysqli_query($query, "SELECT * FROM hasil_tes,pendaftaran WHERE hasil_tes.id_pendaftaran=pendaftaran.id_pendaftaran ORDER BY(hasil_tes.total+0) desc");  
    $no = 1;
    ...
    while($r=mysqli_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
				...
				<td>$r[wawancara]</td>
				<td>$r[total]</td>";
$tampung = mysqli_query($query, "SELECT * FROM daya_tampung");
...
$kapasitas = mysqli_query($query, "SELECT * FROM daya_tampung");
...
if ($no <=$k[kapasitas] AND $total >$t[nilai_minimal]){
  ...
  case "edithasil_tes":
    $edit = mysqli_query($query, "SELECT * FROM hasil_tes,pendaftaran WHERE hasil_tes.id_pendaftaran=pendaftaran.id_pendaftaran AND hasil_tes.id_hasil='$_GET[id]'");
