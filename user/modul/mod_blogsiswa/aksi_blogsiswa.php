...
if ($module=='blogsiswa' AND $act=='hapus'){
  $data=mysqli_fetch_array(mysqli_query($query, "SELECT screenshoot FROM blogsiswa WHERE id_blogsiswa='$_GET[id]'"));
  if ($data['screenshoot']!=''){
  mysqli_query($query, "DELETE FROM blogsiswa WHERE id_blogsiswa='$_GET[id]'");
...
$lokasi_file = $_FILES['fupload']['tmp_name'];
  ...
  if (!empty($lokasi_file)){
    ...
    mysqli_query($query, "INSERT INTO blogsiswa(nama,
		   							   kelas,
	                                   tgl_posting,
                                    	url,
										screenshoot) 
...
elseif ($module=='blogsiswa' AND $act=='update'){
  ...
  $nama_file_unik = $acak.$nama_file; 
   if (empty($lokasi_file)){
    mysqli_query($query, "UPDATE blogsiswa SET nama     = '$_POST[nama]',
									kelas     = '$_POST[kelas]',
                                   url       = '$_POST[url]'
  ...
  header('location:../../media.php?module='.$module);
	...
	$data_gambar = mysqli_query($query, "SELECT screenshoot FROM blogsiswa WHERE id_blogsiswa='$_POST[id]'");
