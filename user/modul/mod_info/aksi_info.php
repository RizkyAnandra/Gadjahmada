...
if ($module=='info' AND $act=='hapus'){
  $data=mysqli_fetch_array(mysqli_query($query, "SELECT gambar FROM info WHERE id_info='$_GET[id]'"));
  if ($data[gambar]!=''){
     mysqli_query($query, "DELETE FROM info WHERE id_info='$_GET[id]'");
...
elseif ($module=='info' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
   ...
   mysqli_query($query, "INSERT INTO info(judul,
	                                       judul_seo,
										   isi_info,
										   tgl_posting,
										   gambar,
...
elseif ($module=='info' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
    mysqli_query($query, "UPDATE info SET judul        = '$_POST[judul]',
                                        judul_seo    = '$judul_seo',
  ...
  header('location:../../media.php?module='.$module);
   ...
   $data_gambar = mysqli_query($query, "SELECT gambar FROM info WHERE id_info='$_POST[id]'");
    ...
    mysqli_query("UPDATE info SET judul        = '$_POST[judul]',
                                          judul_seo    = '$judul_seo',
                                          isi_info  = '$_POST[isi_info]',
