...
switch($_GET[act]){
  ...
  echo"<div class='rightpanel'>
   ...
   <tbody>";
    ...
    $tampil = mysqli_query($query, "SELECT * FROM agenda ORDER BY id_agenda DESC");
	}
	else{
    $tampil=mysqli_query($query, "SELECT * FROM agenda WHERE username='$_SESSION[namauser]' ORDER BY id_berita DESC");}
	 $no = $posisi+1;
  ...
  case "editagenda":
    $edit = mysqli_query($query, "SELECT * FROM agenda WHERE id_agenda='$_GET[id]'");
