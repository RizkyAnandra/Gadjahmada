...
switch($_GET[act]){
  ...
  echo"<div class='rightpanel'>
   ...
   <tbody>";
    ...
    if ($_SESSION[leveluser]=='admin'){
      $tampil=mysqli_query($query, "SELECT * FROM submenu ORDER BY id_sub DESC");
    }
    else{
      $tampil=mysqli_query($query, "SELECT * FROM submenu,menu 
   ...
   <td width=50><center>$g</center></td>
	...
	$menu=mysqli_query($query, "SELECT * FROM menu WHERE id_menu=$r[id_menu]");
 ...
 <option value='' /><b>- Pilih Menu Utama- </b>";
  $menu=mysqli_query($query, "SELECT * FROM menu ORDER BY nama_menu");
   while ($m=mysqli_fetch_array($menu)){
 ...
 echo"</select>
  ...
  case "editsubmenu":
    $edit = mysqli_query($query, "SELECT * FROM submenu WHERE id_sub='$_GET[id]'");
 ...
 <span class='field'>
 <select name='id_menu' class='uniformselect'>";
  $tampil=mysqli_query($query, "SELECT * FROM menu ORDER BY id_menu");
   if ($r[id_menu]==0){
