...
if ($module=='daya_tampung' AND $act=='hapus'){
  $data=mysqli_fetch_array(mysqli_query($query, "SELECT gambar FROM daya_tampung WHERE id_halaman='$_GET[id]'"));
  if ($data['gambar']!=''){
     mysqli_query($query, "DELETE FROM daya_tampung WHERE id_halaman='$_GET[id]'");
...
elseif ($module=='daya_tampung' AND $act=='input'){
  ...
  if (!empty($lokasi_file)){
    ...
    mysqli_query($query, "INSERT INTO daya_tampung(judul,
                                    isi_halaman,
                                    tgl_posting,
                                    gambar) 
                            VALUES('$_POST[judul]',
...
elseif ($module=='daya_tampung' AND $act=='update'){
  ...
  if (empty($lokasi_file)){
    mysqli_query($query, "UPDATE daya_tampung SET kapasitas       = '$_POST[kapasitas]',
  ...
  header('location:../../media.php?module='.$module);
    ...
    UploadBanner($nama_file);
    mysqli_query($query, "UPDATE daya_tampung SET judul       = '$_POST[judul]',
