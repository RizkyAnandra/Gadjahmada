...
echo"<li><a href='media.php?module=home' class='icon-text'>&#8962;</a></li>";        
     $menu=mysqli_query($query, "SELECT * FROM menu WHERE aktif='Y' ORDER BY id_menu ASC");
	...
	$sub=mysqli_query($query,"SELECT * FROM submenu, menu  
    WHERE submenu.id_menu=menu.id_menu AND submenu.id_menu=$r[id_menu] AND submenu.id_submenu=0 AND submenu.aktif='Y' ORDER BY id_sub ASC");
	 ...
	 $sub2 = mysqli_query($query, "SELECT * FROM submenu WHERE id_submenu=$w[id_sub] AND id_submenu!=0");
