    <!-- BEGIN .main-content-left -->
		<div class="main-content-left">
			...
			<?php                    
   			$iklan=mysqli_query($query, "select * from halamanstatis WHERE judul_seo = '$_GET[judul]'"); 
...
<?php
	...
	$detail=mysqli_query($query,"select * from halamanstatis WHERE judul_seo = '$_GET[judul]'"); 
	$d   = mysqli_fetch_array($detail);
	$tgl = tgl_indo($d['tgl_posting']);
	$baca = $d['dibaca']+1;
	if ($d['id_halaman']!=''){
	echo"<div class='main-article-content'>";
	mysqli_query($query, "UPDATE halamanstatis SET dibaca=$d[dibaca]+1 WHERE judul_seo='$_GET[judul]'");
	  if ($d['gambar']!=''){
