  ...
  $u=mysqli_query($query, "UPDATE poling SET rating=rating+1 WHERE id_poling='$_POST[pilihan]'");
  ...
  $jml=mysqli_query($query, "SELECT SUM(rating) as jml_vote FROM poling WHERE aktif='Y'");
  $j=mysqli_fetch_array($jml);  
  $jml_vote=$j[jml_vote];  
  $sql=mysqli_query($query, "SELECT * FROM poling WHERE aktif='Y' and status='Jawaban'");  
